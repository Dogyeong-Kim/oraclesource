...
-- DEPTNO, SALARY 조회
SELECT e.LAST_NAME, e.DEPARTMENT_ID , e.SALARY FROM EMPLOYEES e WHERE e.SALARY = ANY
(SELECT e.SALARY FROM EMPLOYEES e WHERE e.COMMISSION_PCT IS NOT NULL);
...
(SELECT AVG(e.SALARY) FROM EMPLOYEES e) AND INSTR(e.LAST_NAME, 'u') > 0;

-- 각 부서별 평균 연봉보다 더 받는 동일부서 사원들의 LAST_NAME, SALARY,
-- DEPTNO, 해당 부서의 평균연봉 조회(부서별 평균연봉을 기준으로 오름차순)
SELECT e.LAST_NAME, e.SALARY, e.DEPARTMENT_ID FROM EMPLOYEES e WHERE e.SALARY > ANY
...
SELECT e.LAST_NAME, e.SALARY FROM EMPLOYEES e WHERE e.MANAGER_ID = ANY
...
(SELECT e.SALARY FROM EMPLOYEES e WHERE e.LAST_NAME = 'Kochhar') AND e.COMMISSION_PCT =
(SELECT e.COMMISSION_PCT FROM EMPLOYEES e WHERE e.LAST_NAME = 'Hall') AND 
e.LAST_NAME != 'Hall';
...
-- LOCATION 테이블 사용
SELECT l.LOCATION_ID, c.COUNTRY_ID FROM LOCATIONS l JOIN COUNTRIES c ON l.COUNTRY_ID =
c.COUNTRY_ID WHERE l.LOCATION_ID = ANY
...
e.DEPARTMENT_ID, e.SALARY FROM EMPLOYEES e WHERE e.SALARY = ANY
(SELECT e.SALARY FROM EMPLOYEES e JOIN DEPARTMENTS d ON e.DEPARTMENT_ID = d.DEPARTMENT_ID
WHERE d.LOCATION_ID = 1700) AND E.COMMISSION_PCT = ANY (SELECT e.COMMISSION_PCT 
